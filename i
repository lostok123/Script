...
local function deleteSeats(instance)
	for _, child in ipairs(instance:GetChildren()) do
		...
		deleteSeats(child) -- Chamada recursiva para verificar os filhos deste objeto
	end
end
...
function Interagir()
	-- Função para coletar todas as instâncias de ProximityPrompt dentro de uma instância
	local function collectProximityPrompts(instance)
		local proximityPrompts = {} -- Tabela para armazenar os ProximityPrompts

		-- Percorre todos os descendentes da instância
		for _, descendant in ipairs(instance:GetDescendants()) do
			if descendant:IsA("ProximityPrompt") then
				table.insert(proximityPrompts, descendant) -- Adiciona o ProximityPrompt à tabela
			end
		end

		return proximityPrompts -- Retorna a tabela com todos os ProximityPrompts encontrados
	end

	-- Verifica se Construcoes.LocaisRotas existe
	local locaisRotas = game.Workspace.Construcoes.LocaisRotas
	if locaisRotas then
		-- Coleta todas as instâncias de ProximityPrompt dentro de Construcoes.LocaisRotas
		local proximityPrompts = collectProximityPrompts(locaisRotas)

		-- Percorre todas as instâncias de ProximityPrompt coletadas e dispara o evento fireproximityprompt
		for _, proximityPrompt in ipairs(proximityPrompts) do
			fireproximityprompt(proximityPrompt)
		end
	else
		warn("A instância 'Construcoes.LocaisRotas' não foi encontrada.")
	end
end
...
function teste()
	...
	if humanoidRootPart then
		humanoidRootPart.CFrame = humanoidRootPart.CFrame * CFrame.new(0, -50, 0)
	else
		warn("HumanoidRootPart erro.")
	end
end
...
local function teleportWithAnimation(player, destinations, speed)
	...
	if humanoidRootPart then
		for index, destination in ipairs(destinations) do
			...
			wait(duration)
			print("Jogador chegou ao destino " .. index)
			-- Verifica se a posição do destino é diferente das posições SOLO
